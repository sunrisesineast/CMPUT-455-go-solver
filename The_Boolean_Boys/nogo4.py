...
class UCB:
    def __init__(self, sim_num, coefficient=0.4):
        ...
        # use np.append(self.amaf_stats, [[0, 0]], axis=0) to append
        # self.amaf_stats = np.zeros((1, 2))
        self.amaf_stats = np.zeros((49, 2))
    ...
    def compute_ucb(self, num, val, N, index):
        """
        calculate the upper confidence bound
        """
        k = 100
        beta = np.sqrt(k / ((3 * N) + k))
        q_mc = val / num
        q_amaf = self.amaf_stats[index][1] / self.amaf_stats[index][0]
    ...
    def select(self, stats, N):
        ...
        for index, (num, val) in enumerate(stats):
            ...
            ucb = self.compute_ucb(num, val, N, index)
            if ucb > max_val:
                max_val = ucb
                max_index = index
    ...
    def run_ucb(self, board: GoBoard, moves, color):
        ...
        for N in range(1, total_sim + 1):
            ...
            if winner == color:
                ...
                stats[index] += 1
                self.amaf_stats[index] += 1
            else:
                # only increment number of selection
                stats[index][0] += 1
                self.amaf_stats[index][0] += 1
...
def run():
    ...
    """
    board = GoBoard(7)
    con = GtpConnection(UCB(sim_num=10000), board)
